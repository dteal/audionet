...
def define_slim(features_tensor=None, training=False):
    ...
    with slim.arg_scope(
        ...
        net = tf.reshape(features_tensor, [-1, params.NUM_FRAMES, params.NUM_BANDS, 1])
        ...
        net = slim.fully_connected(
            net, params.EMBEDDING_SIZE, scope="fc2", activation_fn=None
        )
        return tf.identity(net, name="embedding")


def load_slim_checkpoint(session, checkpoint_path):
